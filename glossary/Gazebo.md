...
Gazebo is a 3D simulation tool for robotics that includes a physics engine.

It can simulate a robot in different environments while considering physical effects such as gravity, collisions, and inertial properties.

Unlike [[RViz]], which is primarily used for visualization and debugging, Gazebo simulates the physical behavior of the robot.

## Purpose

Gazebo is used to test robotic systems in simulation before deploying them to real hardware.

A robot can be spawned into a simulated environment, controlled through Gazebo systems, and connected to ROS 2 through the `ros_gz_bridge` package.
